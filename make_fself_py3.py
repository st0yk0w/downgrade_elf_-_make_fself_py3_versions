...
import binascii  # for unhexlify in Python 3
...
class ElfEHdr(object):
    ...
    def load(self, f):
        ...
        header_data = f.read(struct.calcsize(ElfEHdr.FMT))
        (self.magic,
         self.machine_class,
         self.data_encoding,
         self.version,
         self.os_abi,
         self.abi_version,
         self.nident_size) = struct.unpack(ElfEHdr.FMT, header_data)
        ...
        ex_data = f.read(struct.calcsize(ElfEHdr.EX_FMT))
        (self.type,
         self.machine,
         self.version,
         self.entry,
         ...
         self.shentsize,
         self.shnum,
         self.shstridx) = struct.unpack(ElfEHdr.EX_FMT, ex_data)

        if self.machine != ElfEHdr.EM_X86_64 or self.version != ElfEHdr.EV_CURRENT:
            raise ElfError('Unsupported machine type or version.')

        if (self.phentsize != struct.calcsize(ElfPHdr.FMT) or
            (self.shentsize > 0 and self.shentsize != struct.calcsize(ElfSHdr.FMT))):
...
class ElfPHdr(object):
    ...
    def load(self, f):
        data = f.read(struct.calcsize(ElfPHdr.FMT))
        (self.type,
         self.flags,
         self.offset,
         self.vaddr,
         ...
         self.filesz,
         self.memsz,
         self.align) = struct.unpack(ElfPHdr.FMT, data)
...
class ElfSHdr(object):
    ...
    def load(self, f):
        data = f.read(struct.calcsize(ElfSHdr.FMT))
        (self.name,
         self.type,
         self.flags,
         self.addr,
         ...
         self.info,
         self.align,
         self.entsize) = struct.unpack(ElfSHdr.FMT, data)
...
class SignedElfExInfo(object):
    ...
    def save(self, f):
        f.write(struct.pack(SignedElfExInfo.FMT,
                            self.paid,
                            self.ptype,
                            self.app_version,
                            self.fw_version,
...
class SignedElfFile(object):
    ...
    def __init__(self, elf, **kwargs):
        ...
        self.paid = kwargs['paid'] if ('paid' in kwargs and kwargs['paid'] is not None) else 0x3100000000000002
        self.ptype = kwargs['ptype'] if ('ptype' in kwargs and kwargs['ptype'] is not None) else SignedElfExInfo.PTYPE_FAKE
        self.app_version = kwargs['app_version'] if ('app_version' in kwargs and kwargs['app_version'] is not None) else 0
        self.fw_version = kwargs['fw_version'] if ('fw_version' in kwargs and kwargs['fw_version'] is not None) else 0
        self.auth_info = kwargs['auth_info'] if ('auth_info' in kwargs and kwargs['auth_info'] is not None) else None
    ...
    def _prepare(self):
        ...
        # compute header_size
        self.header_size = (struct.calcsize(SignedElfFile.COMMON_HEADER_FMT)
                            + struct.calcsize(SignedElfFile.EXT_HEADER_FMT))
        self.header_size += self.num_entries * struct.calcsize(SignedElfEntry.FMT)
        self.header_size += max(
            self.elf.ehdr.ehsize,
            self.elf.ehdr.phoff + self.elf.ehdr.phentsize * self.elf.ehdr.phnum
        )
        ...
        self.meta_size = (self.num_entries * struct.calcsize(SignedElfMetaBlock.FMT)
                          + struct.calcsize(SignedElfMetaFooter.FMT)
                          + SIGNATURE_SIZE)

        self.meta_blocks = []
        for i in range(self.num_entries):
            self.meta_blocks.append(SignedElfMetaBlock())

        self.meta_footer = SignedElfMetaFooter()
        self.meta_footer.unk1 = 0x10000

        if self.auth_info is not None:
            # signature from auth_info
            self.signature = (struct.pack('<QQ', len(self.auth_info), self.ex_info.paid)
                              + self.auth_info[8:]).ljust(SIGNATURE_SIZE, b'\0')
        else:
...
def ensure_hex_string(val, **kwargs):
    ...
    if val_size > 0:
        if val.startswith('0x') or val.startswith('0X'):
            val = val[2:]
        # must be even length and only hex digits
        if (len(val) % 2 != 0) or (not all(x in string.hexdigits for x in val)):
            return None
        val = binascii.unhexlify(val)
        val_size = len(val)

    if exact_size is not None and val_size != exact_size:
        return None
    else:
        if min_size is not None and val_size < min_size:
            return None
        if max_size is not None and val_size > max_size:
            return None

    return val

def input_file_type(val):
    if not (os.access(val, os.F_OK | os.R_OK) and os.path.isfile(val)):
        raise argparse.ArgumentTypeError('invalid input file: {0}'.format(val))
    return val

def output_file_type(val):
    if os.access(val, os.F_OK):
        if (not os.path.isfile(val)) or (not os.access(val, os.F_OK | os.W_OK)):
...
parser.add_argument('--ptype', default=None,
                    help='program type {fake, npdrm_exec, npdrm_dynlib, system_exec, system_dynlib, host_kernel, secure_module, secure_kernel}')
...
if args.ptype is not None:
    ...
    mapped_ptype = ptype_map.get(args.ptype.strip().lower(), None)
    if mapped_ptype is not None:
        ptype = mapped_ptype
    else:
        maybe_val = try_parse_int(args.ptype)
        if maybe_val is None:
            parser.error('invalid program type: {0}'.format(args.ptype))
        ptype = maybe_val
